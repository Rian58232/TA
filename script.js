(() => {
  ...
  function initLoader() {
    ...
    const alreadySeen = sessionStorage.getItem('tatu-v3-loader-seen') === '1';
    const finish = () => {
      loader.classList.add('is-done');
      document.body.classList.add('is-ready');
      sessionStorage.setItem('tatu-v3-loader-seen', '1');
  ...
  function initFloatingWhatsApp() {
    ...
    const observer = new IntersectionObserver(entries => {
      ...
      if (!heroVisible && !expandedOnce) {
        ...
        collapseTimer = window.setTimeout(() => floating.classList.remove('is-expanded'), 5200);
      }
    }, { threshold: .18 });
    ...
    floating.addEventListener('mouseleave', () => {
      collapseTimer = window.setTimeout(() => floating.classList.remove('is-expanded'), 1200);
    });
  }
  ...
  function updateScrollEffects() {
    ...
    if (process && processLine && window.innerWidth > 820) {
